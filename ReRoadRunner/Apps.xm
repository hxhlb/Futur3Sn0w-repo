...
@interface UIWindowScene (RoadRunnerPrivate)
...
@interface UISceneConfiguration (RoadRunnerPrivate)
- (Class)delegateClass;
@end

static UIWindow *RRKeyWindow(void) {
...
%ctor {
    ...
    if (![blacklistedBundleIDs containsObject:bundleID]) {
        ...
        addBecomeActiveObserver(^{
            UIWindow *keyWindow = RRKeyWindow();
            if (!keyWindow) {
                return;
            }
