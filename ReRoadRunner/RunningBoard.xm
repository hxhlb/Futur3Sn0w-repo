...
- (void)nowPlayingAppChanged:(NSString *)bundleID {
    // Enable/disable the terminateWithContext: protection based on whether
    // something is actually playing. Without this, running stays NO (its
    // default) and the hook is permanently a no-op.
    running = (bundleID != nil);
...
- (BOOL)terminateWithContext:(RBSTerminateContext *)context {
    // Log every termination attempt so we can identify what sbreload uses
    HBLogDebug(@"[RR] terminateWithContext: identity=%@ explanation=%@ exceptionCode=0x%x running=%d",
               self.identity.embeddedApplicationIdentifier,
               context.explanation,
               (unsigned int)context.exceptionCode,
               running);
    ...
    if (!validReason) {
        HBLogDebug(@"[RR] terminateWithContext: NOT a valid respring reason, passing through");
        return %orig;
    }
...
%group RBSProcessHandle_iOS16
%hook RBSProcessHandle
// initWithInstance:...cache: intentionally omitted for iOS 16:
// %property booleans default to NO via associated objects, so the
// init hook (which only sets them to NO) is both redundant and
// dangerous -- the ARC setter crashes on partially-init objects.
- (id)initWithRBSXPCCoder:(BSXPCCoder *)coder {
    return initProcessHandleWithCoder(%orig, coder);
}
...
%ctor {
    ...
    if ([%c(RBSProcessHandle) instancesRespondToSelector:@selector(encodeWithBSXPCCoder:)]) {
        ...
        if ([%c(RBSProcessHandle) instancesRespondToSelector:@selector(initWithInstance:auditToken:bundleData:manageFlags:beforeTranslocationBundlePath:executablePath:cache:)]) {
            %init(RBSProcessHandle_iOS16);
        } else {
